...
export class Graph<T extends { id: number }> {
    ...
    removeNode(data: T) {
        ...
        if (nodeToRemove) {
            ...
            nodeToRemove.adjNodes.forEach((edge: ConnectingNode<T>) => {
                //bc undirected, we can just look at the nodToRemove's adjacent nodes
    ...
    removeEdge(source: T, destination: T): void {
        ...
        let destinationNode: Node<T> | undefined = this.nodes.get(destination.id);
