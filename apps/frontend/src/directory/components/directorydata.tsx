...
export async function parseDirectoryData() {
  ...
  // a "slug" of the name (this is to be used in a url).
  const Patriot20: { title: string; slug: string }[] = [];
  const Patriot22: { title: string; slug: string }[] = [];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (buildingNum == 'Patriot-20') {
      Patriot20.push({ title: departmentName, slug: slugVal });
    } else if (buildingNum == 'Patriot-22') {
      Patriot22.push({ title: departmentName, slug: slugVal });
    } else {
      console.log(
